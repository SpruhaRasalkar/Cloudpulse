...
$AUTH_TOKEN = '7da112a13a2e96ed7a5f6f9d94d4461f';  // Replace with your actual Auth token
...
function fetch_cpu_load($hostid, $time_from, $time_till) {
    ...
    return [
        'min' => round(min($values), 2),  // Minimum load
        'avg' => round(array_sum($values) / count($values), 2),  // Average load
        'max' => round(max($values), 2),  // Maximum load
    ];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($hostgroup_ids)) {
        ...
        echo "<ul>
                <li><strong>Start Date:</strong> " . date('Y-m-d', $time_from) . "</li>
                <li><strong>End Date:</strong> " . date('Y-m-d', $time_till) . "</li>
                <li><strong>Host Groups Selected:</strong> ";
